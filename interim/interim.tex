...
Decoders take in the last translated token, or SOS if we are decoding a new
sentence. It applies an embedding layer, followed by an optional dropout.
...
\begin{enumerate}
  ...
  \item No Attention
\end{enumerate}
In the case of attention, we apply one of the attention schemes decribed in
Section ~\ref{sec:attention} to the encoder output, given our current decoding
...
provided. After applying a linear layer and log_softmax, we output our result
for evaluation.
...
\begin{equation}
    ...
    \begin{cases}
        ...
        h_t^T W_a h_s & \text{\emph{general}} \\
...
softmax:
\begin{equation}
  a_t(s) = \frac{\exp(score(h_t, h_s))}{\sum_{s'}\exp(score(h_t, h_s))}
\end{equation}
